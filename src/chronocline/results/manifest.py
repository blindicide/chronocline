...
def create_manifest(
    config_hash: str, experiment: str, seed: int, workers: int, locale: str
) -> dict[str, object]:
...
def finalize_manifest(
    directory: Path, manifest: dict[str, object], status: str = "complete"
) -> None:
    """Atomically write final manifest with checksums of all generated files."""
    checksums = {}
    for path in directory.rglob("*"):
        if path.is_file() and path.name != "manifest.json":
            checksums[str(path.relative_to(directory))] = hashlib.sha256(
                path.read_bytes()
            ).hexdigest()
    manifest.update(
        {
